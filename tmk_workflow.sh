...
./venv/bin/python substitute_tags.py -f filter_params.yaml --lower-sent-start -i tmk_concordance_emtsv.tsv -o tmk_concordance_emtsv_subs.tsv
...
for i in $(seq 9 -1 2); do
    ...
    time (awk -v N="${i}" '{if (NF == $N) print $0}' tmk_concordance_spl_clauses.tsv | \
          ...
          LC_ALL=C.UTF-8 sort --parallel=40 -S 40G -T /data/aramis/tmp | uniq -c | \
          LC_ALL=C.UTF-8 sort -nr -S10G --parallel=5 -T /data/aramis/tmp | \
