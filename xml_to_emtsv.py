...
from bs4 import BeautifulSoup  # Open BeautifulSoup library: File -> Settings -> Build, Execution, Deployment -> /
                               # Project Interpreter -> + -> BeautifulSoup -> Install package


def gen_sents(soup):
    get_heading, find_ref_in_corp, left_cont_name, kwic_name, right_cont_name = identify_sample_type(soup)  # this row /
            # Specifies the corpus from which the sample is taken and names the variables according to this corpus

    yield 'form\n'  # Line breaks
    yield from get_heading(soup)  # Inserts heading
    for line_tag in soup.find_all('line'):

        left_toks, kwic_toks, right_toks = context(line_tag, left_cont_name, kwic_name, right_cont_name)  # Inserts context
...
def identify_sample_type(soup):  # This function specifies from which corpus the sample was taken
    if len(soup.find_all('subquery')) > 0:  # Webcorpus type sample
        ...
        raise ValueError('We could not decide which sample it was')  # Error description;
...
def find_ref_in_webcorpus(line_tag):  # Finds the Webcorpus type references <ref>
    return get_attr_from_tag(line_tag, 'refs')


def find_ref_in_mnsz(line_tag):  # Finds the MNSz type references
...
def get_attr_from_tag(tag, attr_name):   # This function finds the value of a specified attribute of a tag
    value_str = tag.get(attr_name)  #  attr_name is the name of the attribute
    if value_str is None:
        raise ValueError(f'{attr_name} can not be found in {tag.name}!')  # Error description
    return value_str


def get_child(soup, curr_context_str, recursive=False):  # This function finds the children element of a specified tag
    curr_context_tag = soup.find(curr_context_str, recursive=recursive) 
    if curr_context_tag is None:
        raise ValueError(f'{curr_context_str} can not be found in {soup.name} with recusive {recursive}')  # Error description
...
def get_tag_text(curr_context_tag, can_be_empty=False):  # This function finds specified tags
    ...
    if curr_context_string is not None:
        ...
        raise ValueError(f'{curr_context_tag.name} has no string content!')  # Error description
    return ''
